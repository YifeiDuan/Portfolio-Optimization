...
# File      :   prophet_forecaster.py
# Objective :   YK Prophet core.
...
from prophet.plot import (
    ...
    # plot_forecast_component, plot_seasonality, plot_weekly, plot_yearly, seasonality_plot_df
)
...
class Prophet(object):
    ...
    def __init__(
            ...
            special_seasonality="auto",

            holidays=None,

            # add
            holidays_prior_weight="auto",  # "auto", True (config in holidays.csv), False
        ...
        self.special_seasonality = special_seasonality

        self.holidays = holidays

        # added
        self.holidays_prior_weight = holidays_prior_weight
    ...
    def make_holiday_features(self, dates, holidays):
        ...
        for _ix, row in holidays.iterrows():
            ...
            try:
                ...
                additive = bool(row.get('additive', False))
                prior_weight = float(row.get("prior_weight", 1.))
            except ValueError:
                lw = 0
                uw = 0
                additive = False
            ...
            additive_eff = 1 if additive else 2
            if np.isnan(ps):
            ...
            for offset in range(lw, uw + 1):
                ...
                if loc is not None:
                    ...
                    expanded_holidays["additive"][loc] = additive_eff
                    expanded_holidays["prior_weight"][loc] = prior_weight
                else:
                    # Access key to generate value
                    expanded_holidays[key]
                    expanded_holidays["additive"]
                    expanded_holidays["prior_weight"] = np.ones(len(dates))

        # key is column and loc is row,
        # so holiday_features is a (all dates, all holiday columns) dataframe (fill non holidays with 0)
        ...
        prior_scale_list = [
            ...
            for h in holiday_features.columns if h not in ("additive", "prior_weight")
        ]
    ...
    def make_all_seasonality_features(self, df):
        ...
        for name, props in self.seasonalities.items():
            ...
            seasonal_features = pd.concat([seasonal_features, features], axis=1)
            prior_scales.extend(
        ...
        if len(holidays) > 0:
            ...
            # seasonal_features.append(features)
            seasonal_features = pd.concat([seasonal_features, features], axis=1)
            # for those holidays which are not addable with weekly seasonality,
            # remove weekly and special seasonality whenever those holidays encounter the weekends
            week_cols = [col for col in seasonal_features.columns if re.match("weekly", col) is not None]
            special_cols = [col for col in seasonal_features.columns if re.match("special", col) is not None]
            select_col = week_cols + special_cols
            for col in select_col:
                seasonal_features.loc[seasonal_features["additive"] == 2, col] = 0

            # A small prior scale will shrink holiday estimates more than seasonality
            if self.holidays_prior_weight == "auto":
                factor = df['t'] * self.slope + self.intercept
                factor /= factor.min()
                factor = np.array(factor)
            elif self.holidays_prior_weight:  # True
                factor = seasonal_features.prior_weight.values
            else:
                factor = np.ones(df.shape[0])
            for col in seasonal_features.columns:
                seasonal_features[col] = seasonal_features[col].multiply(factor)

            seasonal_features = seasonal_features.drop(["additive", "prior_weight"], axis=1)
        ...
        for name, props in self.extra_regressors.items():
            ...
            seasonal_features = pd.concat([seasonal_features, pd.DataFrame(df[name])], axis=1)
        ...
        if len(seasonal_features) == 0:
            ...
            seasonal_features = pd.concat([seasonal_features,
                                           pd.DataFrame({'zeros': np.zeros(df.shape[0])})], axis=1)
    ...
    def set_auto_seasonalities(self):
        ...
        # monthly seasonality
        monthly_disable = ((last - first) < pd.Timedelta(days=60)) or (min_dt >= pd.Timedelta(days=30))
        fourier_order = self.parse_seasonality_args('monthly', self.monthly_seasonality, monthly_disable, 7)
        if fourier_order > 0:
            self.seasonalities['monthly'] = {'period': 30.4,
                                             'fourier_order': fourier_order,
                                             'prior_scale': self.seasonality_prior_scale,
                                             'mode': self.seasonality_mode,
                                             'condition_name': None}
        ...
        # special seasonality
        special_disable = ((last - first) < pd.Timedelta(days=9))
        fourier_order = self.parse_seasonality_args('special', self.special_seasonality, special_disable, 2)
        if fourier_order > 0:
            self.seasonalities['special'] = {'period': 3.5,
                                             'fourier_order': fourier_order,
                                             'prior_scale': self.seasonality_prior_scale,
                                             'mode': self.seasonality_mode,
                                             'condition_name': None}
    ...
    def linear_fit(df):
        """
        Use linear regression to estimate y and t in df.

        :param df: Data frame with columns ds (date), y_scaled (scaled time series), and t (scaled time)
        :return: A vector with the slope (b) and intercept(t) of the linear regression
        """
    ...
    def fit(self, df, **kwargs):
        ...
        # compute linear regression parameter, the two parameters will be used in both fitting and predicting
        self.slope, self.intercept = self.linear_fit(self.history)
